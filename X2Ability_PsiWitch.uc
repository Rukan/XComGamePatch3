...
simulated function DimensionalRiftStage2_BuildVisualization(XComGameState VisualizeGameState, out array<VisualizationTrack> OutVisualizationTracks)
{
	...
	if( AvatarUnit != none )
	{
		...
		MultiTargetMessageAction.SendMessagesAfterSec = default.DIMENSIONAL_RIFT_STAGE2_NOTIFY_TARGETS_SEC;
	}
	//****************************************************************************************
...
static function X2DataTemplate CreatePsiMindControlAbility()
{
	...
	`CREATE_X2ABILITY_TEMPLATE(Template, default.MindControlAbilityName);
...
static function X2AbilityTemplate CreateTriggerDamagedTeleportListenerAbility()
{
	local X2AbilityTemplate Template;
	local X2AbilityTrigger_EventListener EventListener;
	local X2Effect_RunBehaviorTree DamageTeleportBehaviorEffect;

	`CREATE_X2ABILITY_TEMPLATE(Template, 'TriggerDamagedTeleportListener');
	Template.eAbilityIconBehaviorHUD = EAbilityIconBehavior_NeverShow;
	Template.Hostility = eHostility_Neutral;
...
simulated function TriggerDamagedTeleport_BuildVisualization(XComGameState VisualizeGameState, out array<VisualizationTrack> OutVisualizationTracks)
{
	local XComGameStateHistory History;
	local XComGameStateContext_Ability  AbilityContext;
	local StateObjectReference InteractingUnitRef;
