...
{
	eParcelSizeType_Small,  // SMALLEST_PARCEL_SIZE x SMALLEST_PARCEL_SIZE      (14x14)
	eParcelSizeType_Medium, // SMALLEST_PARCEL_SIZE x 2*SMALLEST_PARCEL_SIZE    (14x28)
	eParcelSizeType_Large,  // 2*SMALLEST_PARCEL_SIZE x 2*SMALLEST_PARCEL_SIZE  (28x28)
};
...
struct native ParcelDefinition
{
	...
	var bool bAllowNonObjectiveOverride; // Normally, ValidObjectiveParcelMissions.Length > 0 means this cannot be used as a normal parcel,
...
var config const float MaxDegreesToSpawnExit;
var config const float MaxDistanceBetweenParcelToPlotPatrolLinks;
var config const float SoldierSpawnSlush;
...
function InitPlacedEvacZone()
{
	...
	foreach `BATTLE.AllActors(class'X2Actor_EvacZone', EvacZone)
	{
		...
		EvacZoneState = XComGameState_EvacZone(NewGameState.CreateStateObject(class'XComGameState_EvacZone'));
		EvacZoneState.SetCenterLocation(WorldData.GetTileCoordinatesFromPosition(EvacZone.Location));
		NewGameState.AddStateObject(EvacZoneState);	
...
function XComGroupSpawn SpawnLevelExit()
{
	...
	BattleDataState = XComGameState_BattleData(NewState.CreateStateObject(class'XComGameState_BattleData', BattleDataState.ObjectID));
	BattleDataState.MapData.SoldierExitLocation = LevelExit.Location;
	NewState.AddStateObject(BattleDataState);
...
function ScoreSoldierSpawn(XComGroupSpawn Spawn)
{
	...
	if( SpawnDistanceSq < Square(ActiveSchedule.MinXComSpawnDistance * class'XComWorldData'.const.WORLD_StepSize) )
	{
		Spawn.Score = 0;
	}
	else
	{
		...
		Spawn.Score = (SpawnDistanceSq / (SpawnDistanceSq + Abs(SpawnDistanceSq - Square(ActiveSchedule.IdealXComSpawnDistance * class'XComWorldData'.const.WORLD_StepSize)))) + (`SYNC_FRAND * SoldierSpawnSlush);
	}
}
...
function ChooseSoldierSpawn()
{
	...
	for(SpawnIndex = 0; SpawnIndex < arrSpawns.Length && SoldierSpawn == none; SpawnIndex++)
	{
		...
		if(!SoldierSpawn.HasValidFloorLocations())
		{
	...
	if(SoldierSpawn == none)
	{
		...
		foreach `XWORLDINFO.AllActors(class'XComGroupSpawn', kSpawn)
		{
			...
			if(SpawnDistanceSq > FurthestSpawnDistanceSq && kSpawn.HasValidFloorLocations())
			{
...
{	
	...
	if (`MAPS.IsStreamingComplete() && IsStreamingComplete())
	{
		...
		class'XComPlotSwapData'.static.AdjustParcelsForBiomeType(BiomeDef.strType, PlotType.strType);
...
{	
	...
	if (`MAPS.IsStreamingComplete() && IsStreamingComplete())
	{
		...
		GeneratingMapPhase = 2;
		GeneratingMapPhaseText = "Loading Map Phase 2: Building World Data";
...
function DebugSoldierSpawns()
{
	...
	MinimumSpawnDistance = ActiveSchedule.MinXComSpawnDistance * class'XComWorldData'.const.WORLD_StepSize;
	...
	IdealSpawnDistance = ActiveSchedule.IdealXComSpawnDistance * class'XComWorldData'.const.WORLD_StepSize;
